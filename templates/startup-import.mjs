...
for (const row of rows) {
  console.log(`${row.fixture} median=${row.medianMs.toFixed(3)}ms p95=${row.p95Ms.toFixed(3)}ms exports=${row.exportCount} status=${row.status}`);
}

if (check) {
  const failures = rows.filter((row) => row.status !== 'ok' || row.p95Ms > row.budgetP95Ms || row.exportCount > row.budgetExports);
...
function discoverEntries(packageJson) {
  ...
  if (packageJson.exports && typeof packageJson.exports === 'object') {
    for (const exportName of Object.keys(packageJson.exports)) {
      ...
      entries.push({ fixture: exportName === '.' ? 'package-root' : 'package-' + exportName.replace(/[^a-z0-9]+/gi, '-') , specifier });
    }
  } else if (packageJson.main) {
    entries.push({ fixture: 'package-main', specifier: pathToFileURL(path.resolve(packageJson.main)).href });
  }
  if (entries.length === 0 && fs.existsSync('src/index.mjs')) {
    entries.push({ fixture: 'source-index', specifier: pathToFileURL(path.resolve('src/index.mjs')).href });
  }
...
function measureEntry(entry, runs) {
  ...
  const problems = [];
  if (p95Ms > entry.budgetP95Ms) problems.push('p95 exceeds ' + entry.budgetP95Ms + 'ms');
  if (exportCount > entry.budgetExports) problems.push('exports exceed ' + entry.budgetExports);
  return {
    category: 'startup',
    fixture: entry.fixture,
    library: 'project',
