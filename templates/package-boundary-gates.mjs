...
const outPath = args.out || 'benchmarks/results/package-boundary-gates-latest.json';
...
for (const row of rows) {
  console.log(`${row.fixture} files=${row.files} bytes=${row.bytes} status=${row.status}`);
}
...
function discoverEntries(packageJson) {
  ...
  if (packageJson.exports && typeof packageJson.exports === 'object') {
    for (const exportName of Object.keys(packageJson.exports)) {
      ...
      if (target) entries.push({ fixture: exportName === '.' ? 'package-root' : 'package-' + exportName.replace(/[^a-z0-9]+/gi, '-'), target });
    }
  }
  if (entries.length === 0 && packageJson.main) entries.push({ fixture: 'package-main', target: packageJson.main });
  if (entries.length === 0 && fs.existsSync('src/index.mjs')) entries.push({ fixture: 'source-index', target: 'src/index.mjs' });
  return entries;
}

function readExportTarget(value) {
  if (typeof value === 'string') return value;
  if (value && typeof value === 'object') return value.import || value.default || value.require || null;
  return null;
}
...
function measureEntry(entry) {
  ...
  const forbidden = readForbiddenPatterns().filter((pattern) => files.some((file) => file.includes(pattern)));
  const maxBytes = Number(args.bytes || 250000);
  ...
  if (forbidden.length) problems.push('forbidden reachable paths: ' + forbidden.join(', '));
  return {
...
function measurePackDryRun() {
  const maxBytes = Number(args.packBytes || 500000);
  ...
  if (result.status !== 0) {
    return {
      ...
      budgetBytes: maxBytes,
      budgetFiles: maxFiles,
      problem: result.stderr.trim() || result.stdout.trim() || 'npm pack failed'
    };
  }
  let pack;
  try {
  ...
  const files = Array.isArray(pack.files) ? pack.files.length : Number(pack.entryCount || 0);
...
function collectReachable(file, reachable) {
  ...
  const importRegex = /\b(?:import|export)\s+(?:[^'"]*?\s+from\s+)?['"]([^'"]+)['"]|import\(['"]([^'"]+)['"]\)/g;
  let match;
...
function resolveLocalImport(dir, specifier) {
  ...
  const candidates = [base, base + '.mjs', base + '.js', base + '.ts', path.join(base, 'index.mjs'), path.join(base, 'index.js'), path.join(base, 'index.ts')];
...
function readForbiddenPatterns() {
  ...
  return String(args.forbid).split(',').map((item) => item.trim()).filter(Boolean);
}
