...
const configPath = path.resolve(args.config || `research/${topic}-sources.json`);
...
const timeoutMs = readInt(args.timeoutMs || args.timeout, 120000);
...
for (const source of manifest.sources) {
  console.log(`${source.name} ${source.type} ${source.status} ${source.localPath || source.error || ''}`);
}
if (manifest.sources.length === 0) {
  console.log('no sources configured in ' + path.relative(process.cwd(), configPath));
}

async function fetchSource(source) {
  const name = sanitizeName(source.name || source.package || source.url || source.path || 'source');
  ...
  try {
    ...
    return { name, type, status: 'unsupported', error: 'unknown source type: ' + type };
  } catch (error) {
    return {
      name,
      type,
...
function fetchGitSource(source, name) {
  ...
  if (!fs.existsSync(localDir)) {
    run('git', ['clone', '--filter=blob:none', '--depth', String(source.depth || 1), source.url, localDir]);
  } else if (args.update) {
    run('git', ['fetch', '--depth', String(source.depth || 1), 'origin'], { cwd: localDir });
  }
  if (source.ref) run('git', ['checkout', source.ref], { cwd: localDir });
  const commit = run('git', ['rev-parse', 'HEAD'], { cwd: localDir, capture: true }).stdout.trim();
  return {
    name,
    type: 'git',
    url: source.url,
...
async function fetchUrlSource(source, name) {
  ...
  const fileName = source.fileName || path.basename(new URL(source.url).pathname) || `${name}.bin`;
...
async function fetchNpmSource(source, name) {
  ...
  const outPath = path.join(dataRoot, sanitizeFileName(packageName) + '.npm.json');
...
function fetchFileSource(source, name) {
  ...
  const to = path.join(dataRoot, source.fileName ? sanitizeFileName(source.fileName) : path.basename(from));
...
function fetchInlineSource(source, name) {
  ...
  const outPath = path.join(dataRoot, sanitizeFileName(source.fileName || name + '.txt'));
  fs.writeFileSync(outPath, text);
  return {
    name,
    type: 'inline',
...
function sanitizeFileName(value) {
  return String(value || 'source').replace(/[/\\:]+/g, '-').replace(/^-|-$/g, '') || 'source';
}
